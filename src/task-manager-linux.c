...
gboolean
get_memory_usage(XtmTaskManager *manager)
{
    FILE *file;
    ...
    gulong mem_avail = 0;
    gulong mem_cached = 0;
    gulong mem_buffers = 0;
    gulong mem_shmem = 0;
    gulong mem_sreclaimable = 0;
    ...
    if ((file = fopen(filename, "r")) != NULL)
    {
        ...
        while (found < 9 && fgets(buffer, sizeof(buffer), file) != NULL)
        {
            ...
            found += !mem_shmem ? sscanf(buffer, "Shmem:\t%lu kB", &mem_shmem) : 0;
            found += !mem_sreclaimable ? sscanf(buffer, "SReclaimable:\t%lu kB", &mem_sreclaimable) : 0;
    ...
    manager->memory_total = mem_total * 1024;

    //manager->memory_used = manager->memory_total - (mem_avail * 1024);
    //mem_used = mem_total - mem_free - mem_cached - mem_buffers - mem_sreclaimable + mem_shmem;

    manager->memory_used =
        manager->memory_total - (mem_free * 1024) - (mem_cached * 1024) - (mem_buffers * 1024) - (mem_sreclaimable * 1024) + (mem_shmem * 1024);

    manager->swap_total = swp_total * 1024;
    manager->swap_free = swp_free * 1024;
